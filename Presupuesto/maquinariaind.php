<?php @session_start(); $title = '- MAQUINARIAIND';
...
#region Permisos
    if(!$rol->getPermissionControl($_SESSION['controles'],Controls::maquinaria->value,$suc)){
        echo "NO TIENES NINGUN TIPO DE PERMISO PARA VER ESTA SECCIÓN EN LA SUCURSAL SELECCIONADA";
        return false;
    }
    $rol->getBranchInPermission($_SESSION['controles'],Controls::maquinaria->value);
         ...
         <div class="modal-body" id="view-maquinarias" style="width:100%;padding:0px">
             ...
             <table class="table table-sm table-bordered txt-11">
                 ...
                 <tbody class="body-table table-maquinarias" >
                    <?php
                        foreach($oMaquinaria->GetDataAll($suc) as $maquinaria){ ?>
                               
                                <tr ondblclick='addMaquinaria(<?php echo $budget; ?>)' id="rowReg-<?php echo $maquinaria['pkmaquinaria']; ?>" onclick="menu(this);">
                                <?php if($budget) { ?>
                                    ...
                                    <?php } ?>
                                    <input type="hidden" name="pkmaquinaria[]" value="<?php echo base64_encode($maquinaria['pkmaquinaria']); ?>">
                                    <td><input type="text" name="maquinariaReg[]" class="form-control" value="<?php echo $maquinaria['nombre']; ?>"></td>
                                    <td><select name="origenReg[]" class="form-control">
                                        <option value=""></option>
                                        <?php
                                            $Nselect = $maquinaria['origen'] == 'NACIONAL' ? 'selected' : '';
                                            $Iselect = $maquinaria['origen'] == 'IMPORTACION' ? 'selected' : '';
                                        ?>
                                            <option <?php echo $Nselect ?> value="NACIONAL">NACIONAL</option>
                                            <option <?php echo $Iselect ?> value="IMPORTACION">IMPORTACIÓN</option>
                                        </select></td>
                                    <td><input type="text" name="precioReg[]" class="form-control" onchange="moneda(this);" value="<?php echo '$'.number_format($maquinaria['costo'], 2, '.', ','); ?>"></td>
                                </tr>
                       <?php } ?>
                    
                     <tr ondblclick='addMaquinaria(<?php echo $budget; ?>)' id="row-0" onclick="menu(this);">
                     <?php if($budget) { ?>
                                        <td width=10px><button class="btn btn-sm btn-info" style="padding: 0px;" title="Agregar a materiales" data-toggle="tooltip" onclick='return add(this,<?php echo json_encode($data); ?>);' ><small class="fa fa-arrow-left" style="font-size:15px;"></small></button></td>
                                    <?php } ?>
                         <td><input type="text" name="maquinaria[]" class="form-control"></td>
...
<?php } ?>
 <?php
    ...
    ?>
 <script type="text/javascript" src="../dependencias/js/Presupuesto/Maquinariaind.js?v=0.0.0"></script>
