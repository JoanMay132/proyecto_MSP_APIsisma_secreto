<?php 
    class Presupuesto extends Conexion{
        ...
        public function Add(array $data): bool{
            $query = self::$conexion->prepare('INSERT INTO presupuesto (
                folio,
                fkcotizacion,
                fkcliente,
                pda,
        ...
        public function GetData($data){
            ...
            $query = self::$conexion->prepare('SELECT * FROM presupuesto,sucursal WHERE presupuesto.pkpresupuesto = :id AND presupuesto.fksucursal = sucursal.pksucursal ');
        ...
        public function GetDataList($sucursal, $anio)
        {
            $query = self::$conexion->prepare('SELECT 
                ...
                presupuesto.fecha,
                presupuesto.pda,
                presupuesto.fkcliente,
                presupuesto.servicio,
                presupuesto.total,
                cliente.pkcliente,
                cliente.nombre AS ncliente
             FROM presupuesto
            LEFT JOIN cliente ON presupuesto.fkcliente = cliente.pkcliente WHERE presupuesto.fksucursal = ? AND YEAR(presupuesto.fecha) = ? ORDER BY CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(folio, "/", 1), "A", -1) AS UNSIGNED) DESC,folio DESC' );
        ...
        public function Delete($data) : bool{
            $query = self::$conexion->prepare('DELETE FROM presupuesto WHERE pkpresupuesto = ?');
        ...
        public function GetTExtra($data){
            ...
            $query = self::$conexion->prepare('SELECT textra,utiempo,costoextra FROM presupuesto WHERE pkpresupuesto = :id');
            $query->bindParam(':id',$data);
            $query->execute();
            $query = $query->fetch(PDO::FETCH_ASSOC);
            return $query;
