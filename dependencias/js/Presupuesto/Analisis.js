...
function dataCotizacion(valor) {
  $.ajax({
    ...
    success: function (respuesta) {
      ...
      OpenWindows("Cargas/Servicios.php?id=" + valor + "", "SERVICIOS")
    }
  });
...
function addPresupuesto(ed = "") {
  ...
  $.ajax({
    type: "POST",
    data: formData,
    url: "Controller/Presupuesto.php",
    dataType: 'json',
    processData: false,
    contentType: false,
    success: async function (respuesta) {
      ...
      if (ed != "") {
        document.location.href = "epresupuesto?edit=" + ed;
      }
      else {

        document.location.href = "epresupuesto?edit=" + respuesta.Presupuesto;
      }
...
async function Carga(suc, cot, cli, cursor = "") {
  ...
  if (window.opener && (window.opener.location.href.includes("ecotizacion") || window.opener.location.href.includes("subcotizacion")) && window.location.href.includes("presupuesto")) {
    ...
    if (cursor != '') {
      ...
      pda = fila.querySelector("[name='pdaReg[]']")?.value || fila.querySelector("[name='pda[]']")?.value
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.ajax({
        ...
        url: "Controller/deleteServicio.php",
        success: async function (respuesta) {
          if(respuesta == ''){
            return false;
          }
...
window.addEventListener('beforeunload', function (event) {
  if (window.opener && (window.opener.location.href.includes("ecotizacion" || "subcotizacion") || window.opener.location.href.includes("subcotizacion"))) {
    if (window.location.href.includes("fila")) {
      ...
      let costo = fila.querySelector("[name='costoReg[]']") ?? fila.querySelector("[name='costo[]']");
      let ContenidoServ = fila.querySelector("[name='contenidoReg[]']") ?? fila.querySelector("[name='contenido[]']");
