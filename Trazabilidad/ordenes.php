<?php @session_start(); {$title = '- ORDENES DE TRABAJO';} //TITULO DE LA PAGINA
    ...
    <main class="main users chart-page" id="skip-target" style="padding:15px;margin-left:0;margin-right:0">
        ...
        <div class="row" style="margin-top:-10px;">
            ...
            <div class="form-group col-12 col-sm-1">
              <?php if($modifica){ ?>
                <a href="javascript:ventana1('addorden','ALTA ORDEN')" class="btn btn-primary btn-sm" style="border-radius:0px;margin-top:5px"><i class="fa fa-plus fa-lg"></i>Nuevo</a>
                <?php } ?>
              </div>
        </div>
        <div class="row" style="margin-top:-15px;">
            <div class="col-12">
                <div style="overflow:auto;height: 83vh;margin-bottom:-58px;background:white;border:1px solid grey" id="display-ordenes" class="contenido">
                    <table class="table-stripped table-hover display-table"  width="100%" id="table" style="font-size:11px;border-collapse: collapse;table-layout: fixed;min-width:900px">
                        <thead style="background:white;position: sticky;top:0;z-index: 10;" >
                          <tr style="border:1px solid #DFDFDF;">
                            ...
                            <th style="width:9%">COTIZACION</th>
                            <th style="width:10%">FECHA</th>
                            <th style="width:40%">CLIENTE</th>
                            <th style="width:34%">ELABORO</th>
                            <th style="width:15%">OC/PO</th>
                          </tr>
                        </thead>
                        <tbody class="body-table" id="orden">
                          <?php foreach($oOt->GetDataJoin($idSuc,$anio) as $result){ ?>
                          <tr style="font-weight:500" onclick="return sel(this);" ondblclick="javascript:ventana1('eorden?edit=<?php echo base64_encode($result['pkorden']); ?>','E-ORDEN')" >
                            ...
                            <?php 
                              $coOc  = $result["ocompra"] != '' ? 'background:#4DB6AC;color:white;border-bottom:1px solid white' : '';
                            ?>
                            <td style="border-right:1px solid #DFDFDF;<?php echo $coOc; ?>"><p style="margin-left:3px"><?php echo $result["ocompra"];?></p></td>
                          </tr>
                            <?php }?>
                        </tbody>
                    </table>
...
function selecciona(){
  ...
  let anio = document.getElementById('anio').value;
    location.href = "ordenes?suc="+suc+"&anio="+anio;
}
